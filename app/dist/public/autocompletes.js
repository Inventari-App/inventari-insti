(() => {
    ...
    } === "function" && define.amd ? define(factory) : (global = typeof globalThis !== "undefined" ? globalThis : global || self, global.autocomplete = factory()));
})(exports, function () {
    "use strict";
    ...
    autocomplete({
        ...
        fetch: function (text, update) {
            window.fetch(optionsRoute).then((res) => res.json()).then((data) => {
                ...
                var suggestions = data.filter((data2) => data2.nom.toLowerCase().includes(text));
                update(filter ? suggestions.filter((suggestion) => suggestion[filter]) : suggestions);
            });
        },
